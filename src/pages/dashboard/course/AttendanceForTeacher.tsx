...
export default function AttendanceForTeacherPageWithAPI() {
    ...
    try {
      ...
      if (response.data.success) {
        const runningSessionCourses = response.data.data.filter(
          (course) =>
            course.runningSession.toLowerCase() ===
            sessionData?.running.toLowerCase()
        );
    ...
    try {
      ...
      if (response.data.success) {
        const filteredStudents = response.data.data.filter(
          (student) =>
            student.runningSession.toLowerCase() ===
            sessionData?.running.toLowerCase()
        );
    ...
    try {
      ...
      if (response.data.message && response.data.data) {
        ...
        if (
          ...
          attendanceRecord.studentList.length > 0
        ) {
  ...
  const loadAttendance = async (): Promise<void> => {
    ...
    try {
      ...
      setTimeout(async () => {
        ...
        const hasExistingAttendance = await fetchAttendanceForDate(
          selectedCourse,
          dateString
        );
        ...
        if (hasExistingAttendance) {
          ...
          if (canEdit) {
            ...
            toast.info("No Previous Attendance", {
              description: `No attendance record found for ${format(
                selectedDate,
                "PPP"
              )}`,
            });
  ...
  const selectedCourseData = courses.find(
    (c) => c.courseId._id === selectedCourse
  );
      ...
      <div className="container mx-auto p-6 space-y-6">
        ...
        <div className="flex items-center space-x-3 mb-8">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              Mark and manage student attendance for {sessionData?.running}{" "}
              session
            </p>
          </div>
        </div>
        ...
        <Card className="shadow-sm">
          <CardHeader>
            ...
            <CardDescription>
              Choose the course and date to view and manage attendance for{" "}
              {sessionData?.running} session
            </CardDescription>
          </CardHeader>
              ...
              <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
                <div>
                  <CardTitle className="flex items-center space-x-2">
                    ...
                    <span>
                      Attendance -{" "}
                      {selectedCourseData?.courseId.code.toUpperCase()}
                    </span>
                  </CardTitle>
                  <CardDescription className="mt-1">
                    {selectedCourseData?.courseId.name} • {sessionData?.running}{" "}
                    • {format(selectedDate, "PPP")}
                  </CardDescription>
                </div>
                      ...
                      >
                        <TableCell className="text-center">
                          {canEdit ? (
                            <Checkbox
                              checked={
                                attendance[student.studentId._id] || false
                              }
                              onCheckedChange={() =>
                                toggleAttendance(student.studentId._id)
                              }
                              className="h-5 w-5"
                            />
                          ) : (
